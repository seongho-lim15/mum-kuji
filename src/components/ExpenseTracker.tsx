...
const ExpenseTracker = () => {
  ...
  const deleteTransaction = async (transactionId: number) => {
    try {
      const response = await fetch(
        `/api/data/transactions?id=${transactionId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleSubmit = async () => {
    if (!formData.unitPrice || !formData.description || !formData.quantity)
      return;
    ...
    if (formData.selectedItem) {
      ...
      const existingItem = itemList.find(
        (item) => item.name === formData.description,
      );
      if (existingItem) {
  ...
  const handleEditSubmit = async () => {
    ...
    const finalAmount =
      editFormData.type === "sale" ? -editTotalAmount : editTotalAmount;

    try {
      const response = await fetch(
        `/api/data/transactions?id=${editingTransaction.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleEditItemSubmit = async () => {
    if (!editItemFormData.name || !editItemFormData.price || !editingItem)
      return;
  ...
  const getFilteredTransactions = () => {
    ...
    if (selectedItemFilter === "전체") {
      ...
      const selectedItem = itemList.find(
        (item) => item.name === selectedItemFilter,
      );
      if (selectedItem) {
      ...
      return timeFilteredTransactions.filter(
        (t) => t.description === selectedItemFilter,
      );
    }
  };
        ...
        <div className="mt-4 text-white">
          ...
          <div className="w-full bg-white bg-opacity-20 rounded-full h-3 mt-2">
            <div
              ...
              style={{
                width: `${thisMonthSpent > budget ? 100 : (thisMonthSpent / budget) * 100}%`,
              }}
            ></div>
          </div>
          <div className="flex justify-between mt-2">
            <div>
              <div className="text-lg font-bold">
                {thisMonthSpent.toLocaleString()}원
              </div>
              <div className="text-xs opacity-90">사용금액</div>
            </div>
            <div className="text-right">
              <div
                className={`text-lg font-bold ${remainingBudget < 0 ? "text-red-200" : ""}`}
              >
                ...
                for (let date = 1; date <= lastDate; date++) {
                  ...
                  days.push(
                    ...
                    >
                      ...
                      {dayTotal !== 0 && (
                        <div
                          className={`font-bold mt-1 text-[10px] leading-tight ${
            ...
            <div className="mb-4">
              ...
              {formData.selectedItem && (
                <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-sm text-blue-700">
                  선택된 품목: {formData.selectedItem.name} (
                  {formData.selectedItem.price.toLocaleString()}원)
                </div>
              )}
            </div>
          ...
          <div className="bg-white w-full rounded-t-lg p-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4">
              ...
              {editFormData.selectedItem && (
                <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-sm text-blue-700">
                  선택된 품목: {editFormData.selectedItem.name} (
                  {editFormData.selectedItem.price.toLocaleString()}원)
                </div>
              )}
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                ...
                <div className="font-bold text-red-600">
                  -{deletingTransaction.amount.toLocaleString()}원
                </div>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={budget}
                  onChange={(e) =>
                    handleBudgetChange(parseInt(e.target.value) || 0)
                  }
                  className="w-full pl-10 pr-4 py-3 border rounded-lg text-right text-xl focus:outline-none focus:border-blue-500"
                />
              </div>
          ...
          <div className="bg-white rounded-lg p-6 w-80">
            <div className="mb-4">
              ...
              <div className="mt-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <div className="text-sm text-gray-500">
                  {deletingItem.price.toLocaleString()}원 •{" "}
