...
export class DataService {
  ...
  // 품목 관리
  static getUserItems = async (email: string): Promise<Item[]> => {
  ...
  static updateUserItem = async (
    email: string,
    itemId: string,
    updatedData: Omit<Item, "id">,
  ): Promise<Item[]> => {
    const items = await this.getUserItems(email);

    // 중복 체크 (자기 자신 제외)
    const duplicateItem = items.find(
      (item) => item.id !== itemId && item.name === updatedData.name,
    );
    if (duplicateItem) {
  ...
  static addUserTransaction = async (
    email: string,
  ...
  static removeUserTransaction = async (
    email: string,
  ...
  static updateUserTransaction = async (
    email: string,
    transactionId: number,
    updatedTransaction: Omit<Transaction, "id" | "timestamp">,
  ): Promise<Transaction[]> => {
    const transactions = await this.getUserTransactions(email);
  ...
  // 설정 관리
  static getUserSettings = async (email: string): Promise<UserSettings> => {
  ...
  // 인증 체크
  static validateUserAccess = async (request: Request): Promise<string> => {
